...
cell4 = list(readRDS('/PHShome/qq06/langenau/projects/01_sc_rms/results/seurat_sara/20191031_MSK72117tencell_seurat-object.rds'),
             readRDS('/PHShome/qq06/langenau/projects/01_sc_rms/results/seurat_sara/C12SC2_seurat-object.rds'))

integration <- function(x, y, label, method='seurat') {
    x$seurat_clusters = x$RNA_snn_res.0.8
    y = FindClusters(y, resolution=0.9)
    y$seurat_clusters = y$RNA_snn_res.0.9
...
set.seed(100)
annotation = read.delim('/PHShome/qq06/langenau/projects/01_sc_rms/final_annotations/Final_clusters_NG.txt', sep='\t', row.names=1, header=T,
...
pdf('MSK72117_UMAP.pdf', width=8, height=5)
