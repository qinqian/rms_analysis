...
Idents(seurat1) <- seurat1@meta.data$seurat_clusters <- seurat1@meta.data[[glue('RNA_snn_res.{args$res}')]]
...
write.table(seurat1.de, file=paste0('results/seurat_sara/', args$label, glue('_SCT_res{args$res}.xls')), sep='\t', quote=F, col.names=NA)

pdf(paste0('results/seurat_sara/', args$label, '_v4_comparison.pdf'), width=14, height=6)
p1 = DimPlot(seurat1, reduction='umap', label=T, group.by=glue('RNA_snn_res.{args$res}')) ## Sara umap with sara cluster
