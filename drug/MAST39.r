...
res <- readRDS('../PDX/results/seurat_sara/MAST-39-Resistant_seurat-object.rds')
sen <- readRDS('../PDX/results/seurat_sara/MAST-39-Sensitive_seurat-object.rds')
res$seurat_clusters = res$RNA_snn_res.0.8
sen$seurat_clusters = sen$RNA_snn_res.0.8

conds = list(res, sen)
objs = integration(conds[[1]], conds[[2]], c('MAST39-Resistant', 'MAST39-Sensitive'))

pdf('ALL_MAST39_batchcorrected_UMAP.pdf', width=8, height=8)
...
pdf('ALL_MAST39_markers.pdf', width=8, height=6)
...
pdf("MAST39_3Dplot.pdf", width=6.8, height=8)
...
pdf('ALL_MAST39_batchcorrected_UMAP_integrative_cluster.pdf', width=8, height=4)
...
write.table(cluster.de.all, file="MAST39_resistant_sensitive_integrative_markers.xls", quote=F,
            sep='\t')

DefaultAssay(objs) <- "RNA"
mean.exp <- colMeans(x = objs@assays$RNA@data[intersect(rownames(objs@assays$RNA@data), paste0("hg19-", muscle)), ], na.rm = TRUE)
...
pdf("MAST39_average_signatures.pdf", width=12, height=4)
...
cluster.de.all$Sensitive_diff_pct = cluster.de.all$`MAST-39-Sensitive_pct.1` - cluster.de.all$`MAST-39-Sensitive_pct.2`

cluster.de.all$Resistant_diff_pct = cluster.de.all$`MAST-39-Resistant_pct.1` - cluster.de.all$`MAST-39-Resistant_pct.2`

clusters = list()
for (i in unique(cluster.de.all$cluster)) {
    genes.sub = subset(cluster.de.all,
                       cluster==i & minimump_p_val<=0.01 & (`MAST-39-Resistant_avg_log2FC` > 0 | `MAST-39-Sensitive_avg_log2FC` > 0) & (Sensitive_diff_pct >= 0.1 | Resistant_diff_pct >= 0.1))
...
write.table(clusters, file="MAST39_resistant_sensitive_integrative_markers_gsea.xls", quote=F,
            sep='\t')

ann_all = read.delim("MAST39/MAST39_integrative_clusters.txt", sep='\t', header=T, row.names=1)
...
## after annotation
pdf('ALL_MAST39_batchcorrected_UMAP_integrative_cluster_allcellstates.pdf', width=8, height=4)
...
pdf('ALL_MAST39_batchcorrected_UMAP_integrative_cluster_dominantstates.pdf', width=8, height=4)
Idents(objs) = objs$seurat_clusters = objs$integrated_snn_res.0.8
...
pdf("MAST39_3Dplot_integrate.pdf", width=8, height=5)
par(mfrow=c(1, 2), cex=0.6)
plotmarkers.integrate(objs[, objs@meta.data$orig.ident=='MAST-39-Resistant'])
plotmarkers.integrate(objs[, objs@meta.data$orig.ident=='MAST-39-Sensitive'])
dev.off()

pdf("ALL_MAST39_integrate_markers.pdf")
...
cpm.sub = cpm[paste0("hg19-", as.character(plot.genes[,2])), ][, objs@meta.data$orig.ident=='MAST-39-Resistant']
sortcells = order(objs[, objs@meta.data$orig.ident=='MAST-39-Resistant']$integrated_snn_res.0.8)
heatdata  <- cpm.sub[, sortcells]
clusters <- objs[, objs@meta.data$orig.ident=='MAST-39-Resistant']$integrated_snn_res.0.8[sortcells]
...
pdf(glue("MAST39_resistant_heatmap.pdf"), width=22, height=12)
set.seed(99)
...
ha = Heatmap(mat2, name = "Scaled Expression",
             ...
             column_title = qq("MAST39 resistant expression for @{ncol(heatdata)} cells"),
...
cpm.sub = cpm[paste0("hg19-", as.character(plot.genes[,2])), ][, objs@meta.data$orig.ident=='MAST-39-Sensitive']
sortcells = order(objs[, objs@meta.data$orig.ident=='MAST-39-Sensitive']$integrated_snn_res.0.8)
heatdata  <- cpm.sub[, sortcells]
clusters <- objs[, objs@meta.data$orig.ident=='MAST-39-Sensitive']$integrated_snn_res.0.8[sortcells]
...
pdf(glue("MAST39_sensitive_heatmap.pdf"), width=22, height=12)
set.seed(99)
topha = HeatmapAnnotation(states=as.vector(anncol[,1]),
                          show_legend=T,
                          gp = gpar(col = NA),
...
for (state in levels(objs$seurat_clusters)) {
    response <- FindMarkers(objs,
                            ident.1 = paste0(state, "_MAST-39-Resistant"),
                            ident.2 = paste0(state, "_MAST-39-Sensitive"),
                            verbose = FALSE)
...
for (state in levels(objs$seurat_clusters)) {
    ...
    write.table(responses.filter[[state]], file=paste0("MAST39_resistant_sensitive_integrative_diffgenes", state, ".xls"), quote=F,
            sep='\t')
}

###pdf(glue("MAST39_differential_markers_EMT.pdf"), width=9, height=28)
...
###pdf(glue("MAST39_differential_markers_Muscle.pdf"), width=9, height=28)
...
pdf(glue("MAST39_differential_markers_Prolif.pdf"), width=9, height=28)
...
pdf(glue("MAST39_differential_markers_Ground.pdf"), width=9, height=28)
