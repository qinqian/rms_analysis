...
#names <- 'SJRHB000026_X1'
#samples <- '/data/langenau/alvin_singlecell/stjude/scRNA-seq/SJRHB000026_X1/'
#doublet <- 'results/SJRHB000026_X1_doublet.csv'
#mixture.samples <- '/data/langenau/alvin_singlecell/stjude/scRNA-seq/SJRHB000026_X1/gem_classification.csv'
...
seurat.obj[["percent.mito"]] <- PercentageFeatureSet(object = seurat.obj, pattern = "^hg19-MT-")
## 2) percentage of ribosomal genes
seurat.obj[["percent.ribo"]] <- PercentageFeatureSet(object = seurat.obj, pattern = "^hg19-RP[SL][[:digit:]]")

## replace with matrix operation 
seurat.obj[["lib.size.10k"]] <- log10(seurat.obj[["nCount_RNA"]]/1e4+1)

if (!is.na(mixture.samples)) {
    mix.mat <- read.csv(mixture.samples[1])
    ## 4) library size for filtering
    mix.mat = mix.mat[match(colnames(seurat.obj), gsub('-1', '', mix.mat$barcode)), ]
    seurat.obj[['fraction.mouse']] = mix.mat[,3]/mix.mat[,2]
}
...
seurat.obj = seurat.obj[grep('^hg19-', rownames(seurat.obj)),]
...
if (!is.na(mixture.samples)) {
    ...
    seurat.obj2@meta.data$mouse <- seurat.obj2@meta.data$fraction.mouse >= 0.05 #& mix.mat$call=='hg19'
    ...
    print(p1)
    dev.off()
    non_mouse = colnames(seurat.obj2)[!seurat.obj2@meta.data$mouse]
    seurat.obj <- subset(seurat.obj, cells=non_mouse)
} else {
    seurat.obj <- subset(x = seurat.obj, subset = nFeature_RNA > 1000 & nFeature_RNA < 8000 & percent.mito < 20)
}
...
FeaturePlot(object = seurat.obj, reduction = "tsne", cols = c("lightgrey", "red"), order = TRUE, 
            features = paste0('hg19-', c("CTSS", "CD68", "HEXB", "MAFB", "LGALS3", "AIF1", "SRGN", "TYROBP", "SLC15A3", "BCL2A1", "GNGT2")))
dev.off()
