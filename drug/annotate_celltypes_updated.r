...
get_args = function(x) {
    ...
    parser$add_argument('--res', dest='res', type='double', default=0.8)
...
#args$seurat <- 'results/seurat_sara/Resistant_seurat-object.rds'
#args$species <- 'human'
#args$label <- 'Resistant'
#args$de <- 'results/seurat_sara/Resistant_SCT_res0.8.xls'
...
get_msigdb = function(category='c2') {
    ...
    for (i in unique(c2[,1])) {
        ylist[[i]] = data.frame(module=i, gene=paste(subset(c2, term==i)$gene, collapse=', '))
    }
...
#saveRDS(allgenes, 'allgenes_signatures.rds')
...
if (args$res != 'NA') {
    primary1_obj$seurat_clusters = primary1_obj[[glue('RNA_snn_res.{args$res}')]]
}
write.table(table(primary1_obj$seurat_clusters), file=glue('{args$label}_res{args$res}_cells_number.xls'), sep='\t', quote=F)
...
cpm.sub = cpm[paste0('hg19-', as.character(plot.genes[,2])), ]
...
pdf(glue("{args$label}_heatmap_res{args$res}.pdf"), width=22, height=12)
set.seed(99)
...
for (i in unique(allmarkers$cluster)) {
   genes = gsub('hg19-', '', as.character(allmarkers[allmarkers$cluster==i, ]$gene))
   if (args$species != 'human') {
...
readr::write_csv(clusters, path=glue('{args$label}_res{args$res}_all_annotation.csv'))

clusters = subset(clusters, p.adjust <= 0.01 & qvalue <= 0.01 & Count >= 3 & FoldEnrichment >= 1.2)
readr::write_csv(clusters, path=glue('{args$label}_res{args$res}_filter_annotation.csv'))
