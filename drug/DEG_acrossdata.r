...
integrated.seurat4 = integration(cell4[[1]], cell4[[2]], c('MAST139-Resistant', 'MAST139-Sensitive'))
...
plots <- list()
for (cell in c("EMT", "Prolif", "Ground", "Muscle", "Interferon", 'Hypoxia')) {
    EMT1 = subset(integrated.seurat4, ident=paste0(cell, '_MAST-139-Resistant'))
    EMT2 = subset(integrated.seurat4, ident=paste0(cell, '_MAST-139-Sensitive'))
    EMT = merge(EMT1, EMT2)
...
ggsave('MAST139_qiqi_target_genes_scatter.pdf', height=12, width=20)

plots <- VlnPlot(integrated.seurat4, features = targets, split.by = "orig.ident",
                 group.by = "cellstate", pt.size = 0, combine = FALSE)
wrap_plots(plots = plots, ncol = 1)
ggsave('MAST139_qiqi_target_genes.pdf', height=25, width=8)


pdf('ALL_MAST139_batchcorrected_targets_umap.pdf', width=12, height=5.5)
...
pdf('ALL_MAST139_batchcorrected_targets_umap_drugmarkers.pdf', width=10, height=10)
...
DEGs = list()
for (cell in c("EMT", "Prolif", "Ground", "Muscle", "Interferon", 'Hypoxia')) {
    response <- FindMarkers(integrated.seurat4, ident.1 = paste0(cell, "_MAST-139-Resistant"), ident.2 = paste0(cell, "_MAST-139-Sensitive"), verbose = FALSE)
...
readr::write_csv(DEGs.df, "MAST139_qiqi_DEG_across_cond.csv")
readr::write_csv(GSEAsig.df, "MAST139_qiqi_GSEAsig_across_cond.csv")
