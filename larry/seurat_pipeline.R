...
all_markers_final = readRDS("final_manual_markers.RDS")
...
ggsave(paste0("Cell_QC_violin_", args$label, ".pdf"), width=8, height=18)

pdf(paste0("Cell_QC_histgram_", args$label, ".pdf"), width=18, height=12)
...
if (args$trans == 'SCT') {
    seurat.obj = process_standard(seurat.obj, norm=F, assaytype=args$assaytype,
                                  output=paste0(args$label, '_pc_jackstraw.pdf'))
} else {
...
pdf(paste0(args$label, "_", args$species, "_umap_vs_tsne.pdf"), width=12, height=5)
...
if (args$species == 'fish') {
    library(ggpubr)
    pdf(paste0(args$label, '_markers_', args$species, '.pdf'), width=30, height=10)
    ...
    library(gridExtra)
    library(grid)
    pdf(paste0(args$label, "nontumor_markers_", args$species, ".pdf"), width=8, height=12)
...
if (length(args$tumor) >= 1) {
    if (args$tumor[1] == -1) {
        print(dim(seurat.obj))
        saveRDS(seurat.obj, paste0(args$label, '_seurat_obj_tumors.rds'))
    } else {
