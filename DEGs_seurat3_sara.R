# adapting sowmya's code to seurat3

# retrieve values from metadata: seurat.obj$RNA_snn_res.0.8
# normalized values are stored in seurat.obj[["RNA"]]@data
# scaled data values are stored in seurat.obj[["RNA"]]@scale.data
# seurat.obj[['pca']] 
...
GeoFeaturePlot <-  function(object, features, reduction.use = "tsne", dim.1 = 1,
  ...
  data.use = data.frame(t(FetchData(object, features, cells = cells.use))) # previously: FetchData(object, features, cells.use = cells.use, use.imputed = use.imputed)
  ...
  data.plot$ident = as.factor(Idents(seurat.obj)) # previously: object@ident
  ...
  #%>%
  # tidyr::separate(sample, c("condition", "group"), remove = FALSE)
  return(data.plot)
}
...
{
  cluster_cells_fg <- WhichCells(seurat_object, idents = forgeround_clusters) # previously: WhichCells(seurat_object, ident = forgeround_clusters)
  control_all <- WhichCells(seurat_object, idents = control_clusters) # previously: WhichCells(seurat_object, ident = control_clusters)
...
{
  raw_data_cluster_cells_fg <- seurat_object[["RNA"]]@counts[rownames(seurat_object[["RNA"]]@data), foreground_cells] # previously: seurat_object@raw.data; seurat_object@data
  raw_data_control_all <- seurat_object[["RNA"]]@counts[rownames(seurat_object[["RNA"]]@data), control_cells] # previously: seurat_object@raw.data; seurat_object@data
  raw_data_cluster_fg_gene_counts <- rowSums(as.matrix(raw_data_cluster_cells_fg)) # previously: rowSums(raw_data_cluster_cells_fg)
  raw_data_all_control_gene_counts <- rowSums(as.matrix(raw_data_control_all)) # previously: rowSums(raw_data_control_all)
  ...
  all_cells_corrected$fg_fraction <- apply(seurat_object[["RNA"]]@data[,foreground_cells],1,function(x)return(length(x[x>1])/length(x)))# previously: seurat_object@data
  all_cells_corrected$bg_fraction <- apply(seurat_object[["RNA"]]@data[,control_cells],1,function(x)return(length(x[x>1])/length(x)))# previously: seurat_object@data
  all_cells_corrected$diff_in_enrichment <- all_cells_corrected$fg_fraction - all_cells_corrected$bg_fraction
  all_cells_corrected$fg_cells_expressing <- apply(seurat_object[["RNA"]]@data[,foreground_cells],1,function(x)return(length(x[x>1])))# previously: seurat_object@data
  all_cells_corrected$bg_cells_expressing <- apply(seurat_object[["RNA"]]@data[,control_cells],1,function(x)return(length(x[x>1])))# previously: seurat_object@data
